...
function ajaxSearch() {
    ...
    if(rem.loadPage == 1 && rem.searchRequest && rem.searchRequest.readyState !== 4) {
        rem.searchRequest.abort();
    }
    ...
    var activeRequest = $.ajax({
        ...
        complete: function(XMLHttpRequest, textStatus) {
            if(rem.searchRequest === activeRequest) {
                rem.searchRequest = null;
                $("body").removeClass("is-searching");
            }
        },  // complete
        success: function(jsonData){
            if(rem.searchRequest !== activeRequest) return;
            if(!$.isArray(jsonData)) {
                jsonData = [];
            }
            ...
            for (var i = 0; i < jsonData.length; i++) {
                no ++;
                var artists = jsonData[i].artist;
                var artistName = $.isArray(artists) ? artists.join(' / ') : artists;
                tempItem =  {
                    id: jsonData[i].id,  // 音乐ID
                    name: jsonData[i].name || '未知歌曲',  // 音乐名字
                    artist: artistName || '未知歌手', // 艺术家名字
                    album: jsonData[i].album || '未知专辑',    // 专辑名字
                    source: jsonData[i].source || rem.source,     // 音乐来源
                    url_id: jsonData[i].url_id,  // 链接ID
                    pic_id: jsonData[i].pic_id,  // 封面ID
                    lyric_id: jsonData[i].lyric_id,  // 歌词ID
                    pic: jsonData[i].pic || null,    // 专辑图片
                    url: jsonData[i].url || null   // mp3链接
                };
            ...
            if(jsonData.length < mkPlayer.loadcount) {
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            if(textStatus === 'abort' || rem.searchRequest !== activeRequest) return;
            $("#list-foot").remove();
            addListbar("error");
            var reason = textStatus === 'timeout' ? '请求超时' : ('服务响应异常 ' + XMLHttpRequest.status);
            layer.msg('搜索失败：' + reason);
            console.error(XMLHttpRequest + textStatus + errorThrown);
        }   // error
    });//ajax
    rem.searchRequest = activeRequest;
}
...
function ajaxLyric(music, callback) {
    if(rem.lyricRequest && rem.lyricRequest.readyState !== 4) {
        rem.lyricRequest.abort();
    }
    lyricTip('歌词加载中...');
    
    if(!music.lyric_id) {
        callback('', music.id, '', music.source);  // 没有歌词ID，直接返回
        return;
    }
    
    var lyricRequest = $.ajax({
        type: "POST",
        url: mkPlayer.api,
        data: {
            types: "lyric",
        ...
        timeout: 15000,
        complete: function() {
            if(rem.lyricRequest === lyricRequest) rem.lyricRequest = null;
        },
        success: function(jsonData){
            if(rem.lyricRequest !== lyricRequest) return;
            // 调试信息输出
            if (mkPlayer.debug) {
            ...
            jsonData = jsonData || {};
            callback(jsonData.lyric || '', music.lyric_id, jsonData.tlyric || '', music.source);    // 原文与翻译歌词
        },   //success
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            if(textStatus === 'abort' || rem.lyricRequest !== lyricRequest) return;
            layer.msg('歌词读取失败 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
            callback('', music.lyric_id, '', music.source);    // 回调函数
        }   // error   
    });//ajax
    rem.lyricRequest = lyricRequest;
}
