...
$(function(){
    ...
    rem.source = playerReaddata('search-source') || 'netease';
    rem.wd = playerReaddata('search-word') || '';
    rem.loadPage = 1;
    ...
    $(".music-list").on("click",".list-loadmore", function() {
        var loadMore = $(this);
        loadMore.html('加载中...').removeClass('list-loadmore list-clickable');
        ajaxSearch();
    });
...
// 展现站内搜索面板
function searchBox() {
    var panel = $("#search-overlay");
    if(!panel.length) {
        layer.msg('搜索面板加载失败，请刷新页面重试');
        return false;
    }
    ...
    $("#search-wd").val(rem.wd);
    $("#music-source input[name='source'][value='" + rem.source + "']").prop("checked", true);
    $("#music-source .source-option").removeClass("is-selected");
    $("#music-source input[name='source']:checked").closest(".source-option").addClass("is-selected");
...
function closeSearchBox() {
    ...
    window.setTimeout(function(){
        if(!panel.hasClass("is-open")) panel.attr("hidden", "hidden");
    }, 320);
}

// 搜索提交
function searchSubmit() {
    var wd = $.trim($("#search-wd").val());
    if(!wd) {
    ...
    playerSavedata('search-source', rem.source);
    playerSavedata('search-word', wd);
...
function addListbar(types) {
    ...
    switch(types) {
        ...
        case "searching": // 搜索中
            html = '<div class="list-item text-center list-status" id="list-foot"><span class="status-spinner"></span>正在搜索音乐...</div>';
        break;

        case "error": // 加载失败
            html = '<div class="list-item text-center list-status list-status-error" id="list-foot">搜索服务暂时不可用，请切换音源后重试</div>';
        break;

        case "noresult": // 没有搜索结果
            html = '<div class="list-item text-center list-status" id="list-foot">没有找到相关歌曲，请更换关键词或音乐源</div>';
        break;
