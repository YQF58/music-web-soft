...
$supported_sources = array('netease', 'tencent', 'kugou');
...
switch(getParam('types'))   // 根据请求的 Api，执行相应操作
{
    ...
    case 'search':  // 搜索歌曲
        $s = getParam('name');  // 歌名
        $s = function_exists('mb_substr') ? mb_substr($s, 0, 80, 'UTF-8') : substr($s, 0, 240);
        $limit = max(1, min(50, intval(getParam('count', 20))));  // 每页显示数量
        $pages = max(1, intval(getParam('pages', 1)));  // 页码
...
{
    if(!$key || !is_string($key)) return $default;
...
function echojson($data)    //json和jsonp通用
{
    ...
    if($callback && preg_match('/^[a-zA-Z_$][0-9a-zA-Z_$\.]*$/', $callback)) //输出jsonp格式
    {
